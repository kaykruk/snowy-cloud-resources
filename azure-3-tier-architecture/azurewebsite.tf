...
resource "azurerm_linux_web_app" "fe-webapp" {
  ...
  app_settings = {
    ...
    "APPINSIGHTS_INSTRUMENTATIONKEY"                  = azurerm_application_insights.snowy-appinsights.instrumentation_key
  ...
  depends_on = [
    azurerm_service_plan.fe-asp,azurerm_application_insights.snowy-appinsights
  ]
}
