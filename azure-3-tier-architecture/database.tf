...
resource "azurerm_mssql_database" "snowy-database" {
  name           = "snowy-db"
  server_id      = azurerm_mssql_server.azuresql.id
  ...
  tags = {
    Application = "snowypaper-demo"
    Env = "Prod"
  }
}

resource "azurerm_key_vault_secret" "sqldb_cnxn" {
  name = "snowysqldbconstring"
  value = "Driver={ODBC Driver 18 for SQL Server};Server=tcp:snowy-sqldb-prod.database.windows.net,1433;Database=snowy-db;Uid=4adminu$er;Pwd=${random_password.randompassword.result};Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30;"
  key_vault_id = azurerm_key_vault.snowy-keyvault.id
  depends_on = [
    azurerm_mssql_database.snowy-database,azurerm_key_vault_access_policy.kv_access_policy_01,azurerm_key_vault_access_policy.kv_access_policy_02,azurerm_key_vault_access_policy.kv_access_policy_03
  ]
}